...
export async function exportFile<T>(type: string, columns: AdjustColumn[], rows: T[], filename: string) {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Data");
  ...
  worksheet.columns = columns
    ...
    .map(({ name }) => ({
      header: name,
      key: name,
      width: (type == "XLSX") ? Math.max(name.length + 2, 15) : undefined,
    }));
    ...
    columns
      ...
      .forEach(({ key, name, type, optlist }) => {
        ...
        if (type === "select" && optlist) {
          ...
          newRow[name0] = val ? new Date(val as string).toISOString() : "";
        } else if (type === "boolean") {
          newRow[name0] = val ? "TRUE" : "FALSE";
        } else {
          newRow[name0] = val ?? "";
...
export async function exportToExcel<T>(
  ...
  worksheet.columns = columns
    ...
    .map(({ name }) => ({
      header: name,
      key: name,
      width: Math.max(name.length + 2, 15), // Đặt độ rộng cột dựa trên độ dài tiêu đề
    }));
